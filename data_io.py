import configparser as ConfigParser    #配置文件解析器 in read_conf()
from optparse import OptionParser  #命令行，添加参数，生成符合unix规范的命令
import numpy as np
...
def read_conf():
 ...
 cfg_file=options.cfg          #提取键cfg下的东西，为传入的参数此段为NONE，为何用这个呢，猜测是能在命令行中使用
 Config = ConfigParser.ConfigParser()
 Config.read(r'cfg/Lbri.cfg')      #读取配置文件

 #[data]
 options.tr_lst=Config.get('data', 'tr_lst')     #添加新的字典键与值，在传入的文件中读取
